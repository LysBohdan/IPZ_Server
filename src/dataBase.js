...
async function getBooking(login) {
    ...
    if (await getData(`Booking/${login}`))
    {
        ...
        const { secondname, firstname, surename, name, room, phone, id} = booking;
...
async function cancelBooking(login, room, name) {
    if (await getData(`Booking/${login}/status`) == "booking")
    {
        await removeData(`Booking/${login}`);
        await removeData(`Booking/${name}/${room}`);
        return 'true';
    }
    else if(await getData(`Booking/${login}/status`) == "settlement"){
        return 'false';
    }else{
